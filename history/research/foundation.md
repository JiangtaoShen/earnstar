...
- **Lower tiers**: repos with 10–99 and 100–999 stars created on 6 random days of the same window, classified by a keyword proxy (`tiers.mjs`), to turn winners-only data into conversion rates (§4).
...
2. **Winning is rare, and rarer without an audience.** Of repos with ≥ 10 stars, 13.7 % reach 100 and 1.7 % reach 1,000. For owners with < 100 followers the rates are 10.2 % and 0.57 %; for everyone else 22.4 % and 4.4 % (§4, L-005).
3. **Archetype shifts the odds about fourfold.** Estimated conversion from ≥ 10 to ≥ 1,000 stars: agent skills 4.2 %, AI-agent tooling and apps 2.8 %, curated resources 2.0 %, self-hosted and desktop apps 2.0 %, non-AI developer tools 1.0 %, playful projects 0.8 % (§4, L-006). Within one problem space, the skill form beats the tool form (anti-slop linters < 40 stars; the humanizer skill 52k).
4. **Cold starts happen, through a borrowed burst.** A quarter of the winners (24 %) belong to users with < 100 followers, and half of those reached 1k within 90 days. They gained a median 260 stars in week one, after one post in a large venue (§5, L-007).
...
6. **Chinese-language reach is a major engine.** 14 % of winners have a Chinese description, and 30 of the 49 cold-start winners have Chinese text in the README. Aggregator accounts such as GitHubDaily pick up repos unprompted.
7. **README polish is table stakes, not an edge.** Fast and slow winners show the same README features (§3, L-008). A good README is necessary; it does not explain who wins.
8. **DEV alone rarely moves a repo.** The median repo gained 1 star in the week after one of DEV's top articles about it (p90: 10); the big gains coincided with HN stories (§6, L-010).
...
## 3. Base rates
<!-- analyze:start -->
_Generated by `node history/research/foundation/analyze.mjs`._
...
| HN story >= 100 points within 7 days of the first star | 3 % | 2,370 / – | – |
...
| [dirac-run/dirac](https://github.com/dirac-run/dirac) | AT | TypeScript | 28 | 25 | 1,514 | HN 393 pts (launch week) | media, 1-line install | Coding Agent singularly focused efficiency and context curation. Reduces API cos |
...
| [browser-use/browser-harness](https://github.com/browser-use/browser-harness) | AT | Python | 4486 (org) | 2 | 18,103 | HN 134 pts (launch week) | media, gif | Browser Harness / Self-healing harness that enables LLMs to complete any task. |
...
<!-- tiers:start -->
_Generated by `node history/research/foundation/tiers.mjs report`._

Random creation days (6): 2025-09-28, 2025-10-12, 2026-01-14, 2026-02-15, 2026-04-05, 2026-06-21. Tier 10-99: 3398 repos; 100-999: 462; >= 1000: the full population (3246).
...
|---|---|---|---|---|
| AI | 23.0 % | 28.1 % | 39.4 % | 1.72 |
| SK | 5.0 % | 11.7 % | 14.9 % | 2.98 |
| CR | 7.3 % | 6.7 % | 8.7 % | 1.19 |
| ML | 3.9 % | 4.5 % | 4.1 % | 1.05 |
| DT | 8.0 % | 6.1 % | 4.8 % | 0.60 |
| SH | 2.0 % | 3.5 % | 2.6 % | 1.31 |
| DA | 2.8 % | 4.1 % | 3.5 % | 1.26 |
| SEC | 2.7 % | 3.5 % | 2.9 % | 1.07 |
| GR | 1.9 % | 3.0 % | 3.4 % | 1.74 |
| FUN | 3.9 % | 2.8 % | 1.9 % | 0.48 |
| HW | 2.4 % | 2.6 % | 1.0 % | 0.41 |
| OTH | 37.1 % | 23.4 % | 12.9 % | 0.35 |
...
Owners that are users with < 100 followers, given >= 10 stars: P(>= 100) 10.2 %, P(>= 1000) 0.57 %. All other owners: P(>= 100) 22.4 %, P(>= 1000) 4.39 %.

Owners that are users with < 100 followers: 10-99 tier 74 % (n 300); 100-999 tier 57 % (n 300); >= 1000 sample 24 % (n 500).
Organization-owned: 10-99 16 %; 100-999 20 %; >= 1000 40 %.
<!-- tiers:end -->
...
- **Launch burst**: their median stars in the first 7 days after the first star is 260 (p25 105, p75 648), and their median single best day is 203 stars. A cold start reaches 1k in 90 days only after a burst of a few hundred stars in the first week.
...
**Patterns**:
- **One strong post in a large community carries a cold start**: a front-page Habr article (anubis), tech press (XDA for tuxmate), Japanese dev sites (Qiita, Zenn), or a big aggregator account on X (GitHubDaily). The reach is borrowed from the venue, not the author.
...
2. **Plan for someone else's amplification.** Our own channels (DEV, awesome lists, GitHub search) are follow-on channels (L-009, L-010). Every cold-start burst studied came from a large venue or another person's post (L-007). The project must therefore be easy to share: a meme-able name, a complaint everyone has, a before/after demo that works as a screenshot, and a one-line install.
...
5. **Set expectations by base rates.** For owners with < 100 followers, 10 % of repos with ≥ 10 stars reach 100 and 0.6 % reach 1,000 (L-005). Predictions in the selection ADR must start from these numbers.
6. **Chinese reach is a lever that needs the owner.** 30 of 49 cold-start winners had Chinese text in the README. Adding it to a program repo is B-class (Constitution §3B) and must be requested through the outbox.
